...
(* Filter identities of every monoidal operation *)
...
(* Process conjunction, assuming DNF form from original formula *)
...
(* Helper unction to compute size of SHpure *)
...
(* Partial preprocess of input: All steps of DNF conversion without expanding (applying distibution) of OR gates *)
...
(* Apply reduction algorithm recursively to conjunction sets *)
...
(* Simplify a conjunction set without assuming any format on given input using information from SHspat *)
